/* =====================================================
   ...
   Clean account integration + savings + referral + support
   + FAQ + Terms + single-overlay behavior
===================================================== */
...
(function () {
    ...
    function myOrders() {
        ...
        return getOrders().filter(
            o => o.email && o.email.toLowerCase() === u.email.toLowerCase()
        );
    }

    window.ensureReferralForUser = function () {
        const u = user();
        if (!u) return null;
        ...
        el.innerHTML = `
            <div class="panel ${className}">
                <div class="panel-header">
                    <div>
                        <div class="panel-eyebrow">${esc(eyebrow)}</div>
                        <h2>${esc(title)}</h2>
                        <p>${esc(subtitle)}</p>
                    </div>
                        ...
                        return `
                            <div class="owned-gift-card">
                                <div class="owned-gift-logo">
                                    ${b?.logo
                                        ? `<img src="${esc(b.logo)}" alt="${esc(o.brand)}" onerror="this.style.display='none';this.parentElement.classList.add('logo-fallback');this.parentElement.setAttribute('data-label','${esc((o.brand || "Gift").slice(0,2).toUpperCase())}')">`
                                        : "🎁"}
                                </div>
    ...
    window.openGiftCardsFromAccount = window.openGiftCards;

    /* =================================================
       SAVINGS
    ================================================= */

    window.getUserSavings = function () {
        const orders = myOrders();
        ...
        makeOverlay(
            ...
            `
                <div class="savings-hero">
                    ...
                    <strong>₹${data.saved.toLocaleString("en-IN")}</strong>
                    <small>Across ${data.orders} order${data.orders === 1 ? "" : "s"}</small>
                </div>

                <div class="savings-stats">
                    <div>
                        <span>Gift card value</span>
                        <strong>₹${data.value.toLocaleString("en-IN")}</strong>
                    </div>
                    <div>
                        <span>Amount paid</span>
                        <strong>₹${data.paid.toLocaleString("en-IN")}</strong>
                    </div>
                    <div>
                        <span>Orders</span>
    ...
    window.openReferral = function () {
        ...
        const data = ensureReferralForUser();
            ...
            `
                <div class="faq-list">
                    <details open>
                        ...
                        <p>Select a gift-card value and the prototype calculates your discounted price before checkout.</p>
                    </details>

                    <details>
                        <summary>Can I use a custom value?</summary>
                        <p>Where enabled, custom values range from ₹100 to ₹10,000.</p>
                    </details>

                    <details>
                        <summary>Where are my gift cards?</summary>
                        <p>Open Account → My Gift Cards after completing a prototype order.</p>
                    </details>

                    <details>
                        <summary>Is this a real payment checkout?</summary>
                        <p>No. This version is a prototype and does not process real payments.</p>
                    </details>

                    <details>
                        <summary>How does Refer & Earn work?</summary>
                        <p>The prototype displays ₹10 for each successful referral. No real money is issued.</p>
                    </details>

                    <details>
                        <summary>How do I contact support?</summary>
                        <p>Open Account → Support and create a ticket.</p>
                    </details>
                </div>
            `,
            "batch-panel"
    ...
    style.textContent = `
        .referral-reward-hero{
            margin-bottom:13px;
            padding:20px;
            text-align:center;
            border-radius:20px;
            background:linear-gradient(135deg,rgba(113,60,243,.16),rgba(65,105,225,.12));
            border:1px solid var(--border);
        }

        .referral-reward-hero span,
        .referral-reward-hero small{
            display:block;
            color:var(--muted);
        }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        setTimeout(function () {
            if (typeof ensureReferralForUser === "function" && user()) {
