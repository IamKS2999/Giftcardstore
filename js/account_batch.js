...
function createHomeSavingsUI() {
    const nav = document.querySelector(".site-header nav");
    const accountButton = document.getElementById("accountButton");
    if (!nav || !accountButton || document.getElementById("homeSavingsButton")) return;

    const button = document.createElement("button");
    button.id = "homeSavingsButton";
    button.className = "home-savings-button";
    button.type = "button";
    button.onclick = function () {
        if (typeof openSavings === "function") openSavings();
    };
    button.innerHTML = `
        <span class="home-savings-label">SAVED</span>
        <strong id="homeSavingsAmount">₹0</strong>
    `;
    nav.insertBefore(button, accountButton);
}

function updateHomeSavings() {
    const button = document.getElementById("homeSavingsButton");
    if (!button) return;
    ...
    const amount = document.getElementById("homeSavingsAmount");
    if (!amount) return;
    ...
    amount.textContent = "₹" + saved.toLocaleString("en-IN");
}
...
function openProfileSection() {
    ...
    const phoneText = user.phone ? "+91 " + escapeHTML(user.phone) : "Not added";
    const verificationControl = verified
        ? `<strong class="verified-text">✓ Verified</strong>`
        : `<button class="verify-mobile-button" onclick="openPhoneVerification()">Verify mobile</button>`;
    ...
    makeAccountDetailOverlay(
        ...
        `
            <div class="profile-detail-grid">
                ...
                <div><span>Mobile</span><strong>${phoneText}</strong></div>
                <div><span>Mobile verification</span>${verificationControl}</div>
            </div>
            <div class="settings-note">Verify your mobile number using the demo OTP. No real SMS is sent in this prototype.</div>
        `
    );
}

let profileDemoOtp = null;
...
function openPhoneVerification() {
    ...
    const old = document.getElementById("phoneVerificationOverlay");
    if (old) old.remove();

    const overlay = document.createElement("div");
    overlay.id = "phoneVerificationOverlay";
    overlay.className = "overlay";
    overlay.style.display = "flex";

    overlay.innerHTML = `
        <div class="panel account-detail-panel phone-verification-panel">
            <div class="panel-header">
                <div>
                    <div class="panel-eyebrow">Verification</div>
                    <h2>Verify mobile number</h2>
                    <p>Confirm the number linked to your account.</p>
                </div>
                <button class="close-button" onclick="closePhoneVerification()">×</button>
            </div>

            <label class="field-label">Mobile number</label>
            <div class="verification-phone-row">
                <span>+91</span>
                <input id="profilePhone" class="login-input" type="tel" inputmode="numeric" maxlength="10" placeholder="10-digit mobile number" value="${escapeHTML(user.phone || "")}">
                <button class="otp-button" onclick="sendProfileDemoOTP()">Send OTP</button>
            </div>

            <div id="profileOtpStatus" class="otp-status">Send an OTP to continue.</div>

            <div id="profileOtpArea" style="display:none">
                <label class="field-label">Enter OTP</label>
                <input id="profileOtp" class="login-input" type="tel" inputmode="numeric" maxlength="6" placeholder="6-digit OTP">
                <div id="profileOtpNote" class="demo-otp-note"></div>
            </div>

            <div id="profileVerifyError" class="login-error"></div>
            <button class="wide-primary" onclick="completePhoneVerification()">Verify mobile</button>

            <div class="prototype-note">Prototype verification only. No real SMS is sent.</div>
        </div>
    `;
...
function sendProfileDemoOTP() {
    const phone = (document.getElementById("profilePhone")?.value || "").replace(/\D/g, "");
    const error = document.getElementById("profileVerifyError");
    ...
    profileDemoOtp = String(Math.floor(100000 + Math.random() * 900000));
    document.getElementById("profileOtpArea").style.display = "block";
    document.getElementById("profileOtpStatus").textContent = "OTP generated for this prototype.";
    document.getElementById("profileOtpNote").textContent = "Demo OTP: " + profileDemoOtp + " • No real SMS is sent.";
    error.style.display = "none";
}
...
function completePhoneVerification() {
    ...
    if (!profileDemoOtp) {
        error.textContent = "Send the OTP first.";
        error.style.display = "block";
        return;
    }

    if (otp !== profileDemoOtp) {
        error.textContent = "Incorrect OTP. Enter the generated demo OTP.";
        error.style.display = "block";
        return;
    }

    saveUser({ ...user, phone, phoneVerified: true });
    profileDemoOtp = null;
    closePhoneVerification();
    showNotice("Your mobile number has been verified in this prototype.", "Mobile verified", "success");
}

function closePhoneVerification() {
    const el = document.getElementById("phoneVerificationOverlay");
    if (el) el.style.display = "none";
}
...
.settings-section-clean{
        ...
        .home-savings-button{
            ...
            border:1px solid rgba(113,60,243,.35);
            border-radius:13px;
            background:linear-gradient(135deg,rgba(113,60,243,.14),rgba(65,105,225,.12));
            color:var(--text);
            cursor:pointer;
        }
        ...
        .verification-phone-row>span{
            color:var(--muted);
            font-weight:800;
        }

        .verification-phone-row .login-input{
            margin:0;
            border:0;
            box-shadow:none!important;
            background:transparent;
        ...
        @media(max-width:700px){
            ...
            .verification-phone-row .otp-button{
                grid-column:1 / -1;
                width:100%;
                margin:0 6px 6px 0;
            }
